...
class OdometryVisualizer:
    ...
    def __init__(self, title: str = "Odometry Path", start_zone_rect: list = [0.55, 1.5]):
        """
    ...
    def update_plot(self, positions: List[Tuple[float, float]]):
        ...
        if len(positions) > 0:
            ...
            self.ax.add_patch(
                plt.Rectangle((0 - self.start_zone_rect_x, 0 - self.start_zone_rect_y), 2 * self.start_zone_rect_x, 2 * self.start_zone_rect_y, color="green", alpha=0.3)
            )
            ...
            if lap > 0:
                ...
                if len(lap_samples[lap]) > 6:
                    ...
                    if drift_mag < MAX_DRIFT_THRESHOLD:                        
                        ...
                        smoothed_drift_x = alpha * new_drift_x + (1 - alpha) * smoothed_drift_x
                        smoothed_drift_y = alpha * new_drift_y + (1 - alpha) * smoothed_drift_y
